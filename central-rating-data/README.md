...
- Exact lifetime-high requests remain local: the companion reads this addon's own SavedVariables queue and contacts IronForge directly. In v1.4, priority players viewed within three days and local characters refresh daily; other profiles refresh weekly. Each run is capped at 50 profiles, with failures rotated behind untried requests.
